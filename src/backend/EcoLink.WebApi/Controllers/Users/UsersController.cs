...
public class UsersController(IMediator mediator) : BaseController
{
    ...
    public async Task<IActionResult> Create(CreateUserWithReturnCommand command, CancellationToken cancellationToken = default)
        => Ok(await mediator.Send(new CreateUserWithReturnCommand(command), cancellationToken));

    [HttpPut("update")]
    [ProducesResponseType(typeof(int), StatusCodes.Status200OK)]
    public async Task<IActionResult> Update(UpdateUserCommand command, CancellationToken cancellationToken = default)
    ...
    [ProducesResponseType(typeof(UserResultDto), StatusCodes.Status200OK)]
    public async Task<IActionResult> Get(long userId, CancellationToken cancellationToken = default)
        => Ok(await mediator.Send(new GetUserQuery(userId), cancellationToken));

    [HttpGet("get-by-telegram-id/{telegramId:long}")]
    [ProducesResponseType(typeof(UserResultDto), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetByTelegramId(long telegramId, CancellationToken cancellationToken = default)
    ...
    public async Task<IActionResult> GetForApplication(CancellationToken cancellationToken = default)
